...
import { EvaluateBodySchema } from "@/lib/validation";
import { z } from "zod";

type EvaluateBody = z.infer<typeof EvaluateBodySchema>;

export async function POST(req: NextRequest) {
  return withAuth<EvaluateBody>(
    req,
    { role: "admin", bodySchema: EvaluateBodySchema },
    async ({ body }) => {
      const res = await fetch(`${env.RAG_URL}/evaluate`, {
        method: "POST",
      ...
      const evalRun = await prisma.evalRun.create({
        data: {
          ...
          sourceFilter: body!.source_filter ? body!.source_filter.join(", ") : null,
      ...
      return Response.json({ result: r, evalRunId: evalRun.id });
    },
  ).catch((err) => {
    logError("admin.evaluate.uncaught", err);
